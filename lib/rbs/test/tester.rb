module RBS
  module Test
    class Tester
      ...
      def install!(klass, sample_size:, ignored_classes:)
        ...
        builder.build_instance(type_name).tap do |definition|
          ...
          tester, set = instance_testers[klass] ||= [
            MethodCallTester.new(klass, builder, definition, kind: :instance, sample_size: sample_size, ignored_classes: ignored_classes),
            Set[]
          ]
        ...
        builder.build_singleton(type_name).tap do |definition|
          ...
          tester, set = singleton_testers[klass] ||= [
            MethodCallTester.new(klass.singleton_class, builder, definition, kind: :singleton, sample_size: sample_size, ignored_classes: ignored_classes),
            Set[]
          ]
      ...
      class MethodCallTester
        ...
        def initialize(self_class, builder, definition, kind:, sample_size:, ignored_classes:)
        ...
        def check
          @check ||= TypeCheck.new(self_class: self_class, builder: builder, sample_size: sample_size, ignored_classes: ignored_classes)
        end
