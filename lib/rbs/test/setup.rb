...
begin
  ...
  ignored_classes = (ENV['RBS_TEST_IGNORED_CLASSES'] || '').split(',').map! { |ignored_class| ignored_class.strip }.push(double_class).compact!
...
if filter.empty?
  ...
  STDERR.puts "  [OPTIONAL] RBS_TEST_IGNORED_CLASSES: sets the classes to be ignored"
  exit 1
end
  ...
  if class_name
    if filter.any? {|f| match(to_absolute_typename(f).to_s, class_name.to_s) } && skips.none? {|f| match(f, class_name.to_s) }
      if env.class_decls.key?(class_name)
        ...
        tester.install!(tp.self, sample_size: sample_size, ignored_classes: ignored_classes)
      end
    end
  end
end
