...
class RBS::TestTest < Minitest::Test
  include TestHelper
...
EOF
      manager.build do |env|
        ...
        hook = RBS::Test::Hook.install(env, klass, logger: logger)
...
EOF
      manager.build do |env|
        ...
        hook = RBS::Test::Hook.install(env, klass, logger: logger).verify_all
...
EOF
      manager.build do |env|
        typecheck = RBS::Test::TypeCheck.new(self_class: Integer, builder: DefinitionBuilder.new(env: env))
...
EOF
      manager.build do |env|
        typecheck = RBS::Test::TypeCheck.new(self_class: Object, builder: DefinitionBuilder.new(env: env))
  ...
  def test_verify_error
    SignatureManager.new do |manager|
      manager.build do |env|
        ...
        assert_raises(RBS::Test::Hook::Error) do
          hook.run do
